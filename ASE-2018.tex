...
The rest of the paper is structured as follows. Section \ref{sec:hybrid_persistence} introduces a running example and provides a brief introduction to CBP.
Section \ref{sec:hybrid_persistence} presents the approach to speed up model loading and its supporting data structures. Section \ref{sec:evaluation} presents experimental results and evaluation. Section \ref{sec:related_work} provides an overview of related work, and Section \ref{sec:conlcusions_and_future_work} concludes with a discussion on directions of future work.


\section{Hybrid Persistence}
\label{sec:hybrid_persistence}
In this work, we augment Epsilon CBP \cite{DBLP:conf/models/YohannisKP17} with NeoEMF \cite{daniel2016neoemf}. The former is the change-based representation while the latter is the state-based representation. Changes made to a model are recorded into both representations: the former persists all the changes while the latter only persists the model's last state produced by the changes. However, only the state-based persistence is used for loading and querying the model. 
