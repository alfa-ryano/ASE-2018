...
\begin{lstlisting}[style=eol,caption={The text change-based persistence of producing the state-based model in List. \ref{lst:xmimodel2}.},label=lst:cbpmodel]
session 1
...
%In this section, we present the known limitations of the hybrid model persistence.
The hybrid model persistence is built upon the Eclipse Modeling Framework. Thus, the change-based component can support persisting information about the subsets of Ecore, such as classes, single and multi-valued features (attributes and references), datatypes, and containment. The change-based component supports persisting changes of models in the form of atomic events and sessions. A session is a course from starting editing a model (after load or save) till the next save/close event. 
Events can be creating and deleting an object, setting and unsetting a feature, moving a value or object within a multi-valued feature, and removing/adding a value or object from/to a feature. 

Moreover, the hybrid model persistence only supports single-file models; it cannot handle models persisted in multiple files/backends. It means that it can only persist changes from one state-based representation to a single change-based representation. The hybrid model persistence does not have support for detection in case the state-based and change-based representation fall out. 
