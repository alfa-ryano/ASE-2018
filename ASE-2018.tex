...
\begin{abstract}
\label{abstract}
Change-based approach comes with the advantages of carrying more accurate semantic information of the evolution of technological artefacts and thus can be potentially used for faster and more accurate model comparison, merging, and analytics. On the other hand, state-based model persistence is growing on its capabilities to process large-scale models by harnessing the power of big data backend. In this work, we integrate both persistence into one single approach, a hybrid model persistence, in order to gain the benefits of both. In this  paper, we present the design of our hybrid model persistence and report its impact on time and memory footprint for model loading and saving, storage space usage, and detecting changes of versions of the same model. We conclude that hybrid model persistence can provide considerable benefits on model loading time and change detection with an acceptable trade-off on memory footprint and storage space usage. 
\end{abstract}
%
% no keywords
\begin{IEEEkeywords} 
